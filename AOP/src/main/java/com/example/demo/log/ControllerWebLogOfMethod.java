...
@Target(ElementType.METHOD)             //功能：指明了修饰的这个注解的使用范围，即被描述的注解可以用在哪里。  这里 METHOD代表只能用在 “方法”上，即函数
public @interface ControllerWebLogOfMethod {    //@interface Annotation{ } 定义一个注解 @Annotation，一个注解是一个类
    //这里是使用注解时的两个参数。
